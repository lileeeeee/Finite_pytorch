...
def flatten_sum(logps):
    ...
    return torch.sum(logps, [1], keepdim=True)


def gaussian_diag():
    class o(object): pass

    o.logps = lambda x: -0.5 * (torch.log(2. * np.pi) + x ** 2)
    o.logp = lambda x: flatten_sum(o.logps(x))
    o.logps_g = lambda x, mean, logsd: -0.5 * (
                torch.log(2. * np.pi) + 2. * logsd + (x - mean) ** 2 / torch.exp(2. * logsd))
...
def log_standard_Gaussian(x):
    return flatten_sum(-0.5 * (math.log(2. * np.pi) + x ** 2))


# logistic distribution
def log_logistic(x):
    s = 2.0
    return flatten_sum(-x / s - torch.log(s) - 2.0 * torch.log(1.0 + torch.exp(-x / s)))
